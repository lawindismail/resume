...
\renewcommand{\arraystretch}{1.4} % Increase vertical spacing between rows
\begin{tabularx}{\textwidth}{@{} >{\bfseries}l X @{}}
    Systems Programming \& Software: & C, C++, Python, MATLAB, Simulink, AVR Assembly, Multithreaded Processing (PThreads), Git (GitHub), Linux, Visual Studio Code \\
    ...
    Automation, Controls \& Simulation: & Finite Element Analysis (FEA), Topology Optimisation (SOLIDWORKS \& nTop), ANSYS Granta Selector, Rockwell Arena, Control Systems, PLC (Programmable Logic Controllers), NI MyRIO, NI LabVIEW \\
...
\textit{Graduated with First Class Honours; Weighted Average Mark (WAM): 81; Final Year GPA: 3.8}
\newpage % This is because work experience is in the final line of page one
% ===== WORK EXPERIENCE =====
\section{Work Experience}
\textbf{Hardware Engineer} \textbar{} \textit{DataFarm (Startup)} \hfill
...
\begin{itemize}
    ...
    \item Engineered low-level systems firmware in C/C++ for ESP32 architecture; utilized multithreaded processing (PThreads) and optimized sleep-state power management for resource-constrained hardware.
...
\subsection{Industrial Automation System --- Capstone with Sutton Tools Pty Ltd}
\begin{itemize}
    \item Developed an end-to-end automated defect detection and feeder system for HSS drill bits to replace manual inspection, matching the 30-second CNC manufacturing cycle.
    \item Implemented an unsupervised \textbf{Machine Learning pipeline using the PaDiM framework} with a \textbf{ConvNeXt-Tiny backbone}; achieved a \textbf{0.9994 AUROC} and \textbf{0.9894 F1-score} in identifying surface anomalies and geometric defects.
    \item Engineered ESP32 firmware using \textbf{FreeRTOS} to manage a mutex-protected queue, tracking four physical parts simultaneously through loading, cleaning, inspection, and sorting stations.
    \item Designed a \textbf{4-station rotary indexing fixture} in Onshape, utilizing a NEMA 17 stepper motor with a 4:1 gear ratio and a spring-loaded mechanism for stabilized 360$^\circ$ drill rotation during imaging.
    \item Developed a robust communication handshake (GPIO/Serial) between the ESP32 controller and a Raspberry Pi inference engine to synchronize multi-view image capture with mechanical motion.
    \item Integrated physical classification hardware, including a servo-actuated diverter and a magnetic drop-gate, to automate the binning of parts based on real-time ML inference results.
    \item Applied digital signal processing (rolling median filters) to Hall-effect sensor data to ensure reliable part detection in high-vibration industrial environments.
\end{itemize}
...
\begin{itemize}
    ...
    \item Developed automated control algorithms using closed-loop PID logic to regulate robotic actuation with $\pm 0.5$ mm precision; calibrated load cells to $\pm 0.1$ N and IR sensors to $\pm 2$ mm.
...
\begin{flushleft}
    \begin{tabular}{@{}lll@{}}
        Arijana Kekoc & Student at RMIT University & 0457 195 599 \\
        Dr. Amirali Khodadadian Gostar  & Senior Lecturer at RMIT University  & 0432 568 428 \\
        Dhanuka Amarasinghe & Student at RMIT University & 0416 312 460 \\
        Mohnish Deshpande & Software Engineer at DataFarm Technologies & 0480 136 857 \\
    \end{tabular}
\end{flushleft}
